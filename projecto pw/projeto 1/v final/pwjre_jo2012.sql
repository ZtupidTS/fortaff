...
--
-- Host: localhost
-- Generation Time: Dec 03, 2011 at 01:41 AM
...
CREATE TABLE IF NOT EXISTS `atleta` (
  ...
  `old_grupo_sanguineo` varchar(3) CHARACTER SET utf8mb4 DEFAULT NULL,
...
INSERT INTO `classificacao_evento` (`cod_classificacao_evento`, `cod_evento`, `classificacao`) VALUES
(1, 1, 5),
(2, 2, 3),
(3, 3, 1),
(7, 2, 5),
...
(2, 'co', 'comite_organizacao', 'jre', 'jre', 'jre123456', 'V', '2011-12-03', 'ricain@iol.pt'),
(3, 'es', 'Espanha', 'Juan', 'Juan_de_espanha', 'as', 'X', NULL, 'juan@hotmail.com'),
(4, 'fr', 'França', 'Dominic', 'dom2', 'dom', 'V', '2011-12-03', 'dom@msn.com'),
(5, 'pt', 'Portugal', 'Alfredo', 'alf', 'alf', 'V', '2011-12-03', 'alf@sapo.pt'),
...
(10, 'af', 'Afeganistão', 'Hamed', 'hamed', 'hamed', 'V', NULL, 'hamed@gmail.com'),
...
(14, 'cv', 'Cabo Verde', 'Joaquim D''Almeida', 'ja', '5bd22ea425', 'V', NULL, 'ja@gmail.com');
...
INSERT INTO `evento` (`cod_evento`, `designacao`, `descricao`, `data`, `hora`, `duracao_estimada`, `preco`, `lugares_total`, `lugares_reservados`, `estado_valido`) VALUES
(1, 'Cerimonia', 'Abertura', '2012-07-27', '20:00:00', '02:30:00', 9.99, 1000, 500, 'V'),
(2, 'Cerimonia', 'Encerramento', '2012-08-12', '20:00:00', '01:30:00', 9.99, 1000, 50, 'V'),
(3, 'Cerimonia', 'Entrega de Medalhas 200M', '2012-08-02', '19:00:00', '01:00:00', 9.99, 1000, 150, 'V'),
(7, 'Cerimonia', 'Fecho', '2012-10-10', '11:00:00', '02:00:00', 12.00, 20000, 0, 'X'),
(8, 'Cerimonia', 'Bandeira', '2012-07-28', '12:00:00', '02:00:00', 0.00, 200, 0, 'V');
...
--
-- Stand-in structure for view `lugares_vazios`
--
CREATE TABLE IF NOT EXISTS `lugares_vazios` (
`cod_prova` int(11)
...
INSERT INTO `prova` (`cod_prova`, `cod_modalidade`, `local`, `data`, `hora_inicio`, `duracao`, `nome_juiz`, `preco`, `lugares_total`, `lugares_reservados`, `estado_valido`, `sexo`) VALUES
(13, 11, 'Londres', '2012-07-28', '16:00:00', '01:00:00', 'Walter', 12.00, 10000, 0, 'V', 'M'),
(16, 2, 'Londres', '2012-07-30', '16:00:00', '03:00:00', 'Richard', 10.00, 10000, 100, 'V', 'F'),
(18, 23, 'Londres', '2012-08-12', '14:25:00', '04:00:00', 'Filipe', 12.00, 12000, 0, 'V', 'F'),
(19, 12, 'Londres', '2012-08-09', '15:00:00', '02:00:00', 'Richard', 12.00, 40000, 0, 'V', 'F'),
(20, 2, 'Londres', '2012-08-06', '17:00:00', '03:00:00', 'Simons', 15.00, 20000, 0, 'V', 'M'),
(21, 5, 'Londres', '2012-08-06', '11:00:00', '02:00:00', 'Philips', 8.90, 5000, 0, 'V', 'M'),
(22, 19, 'Londres', '2012-08-06', '14:00:00', '04:00:00', 'Simons', 14.90, 35000, 0, 'V', 'M'),
(23, 25, 'Londres', '2012-08-02', '15:00:00', '03:00:00', 'Alfred', 9.90, 30000, 0, 'V', 'M'),
(24, 12, 'Londres', '2012-08-05', '15:30:00', '02:00:00', 'Christian', 19.90, 55000, 0, 'V', 'M'),
(25, 28, 'Londres', '2012-08-06', '10:30:00', '02:00:00', 'smith', 9.90, 10000, 0, 'V', 'M'),
(26, 19, 'Londres', '2012-07-29', '16:00:00', '02:00:00', 'Walter', 14.90, 35000, 0, 'V', 'F');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `classificacao_media_evento` AS select `ev`.`cod_evento` AS `cod_evento`,(sum(`cl`.`classificacao`) / count(0)) AS `classif_media` from (`evento` `ev` join `classificacao_evento` `cl`) where (`ev`.`cod_evento` = `cl`.`cod_evento`) group by `ev`.`cod_evento`;
...
--
-- Structure for view `lugares_vazios`
--
DROP TABLE IF EXISTS `lugares_vazios`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `lugares_vazios` AS select `prova`.`cod_prova` AS `cod_prova`,(`prova`.`lugares_total` - `prova`.`lugares_reservados`) AS `lugar_vazios` from `prova` WITH CASCADED CHECK OPTION;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `lugares_vazios_evento` AS select `evento`.`cod_evento` AS `cod_evento`,(`evento`.`lugares_total` - `evento`.`lugares_reservados`) AS `lugar_vazios` from `evento` WITH CASCADED CHECK OPTION;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `total_elemento_equipa` AS select `elemento_in_equipa`.`cod_equipa` AS `cod_equipa`,count(0) AS `total` from `elemento_in_equipa` where (`elemento_in_equipa`.`estado_valido` <> 'X') group by `elemento_in_equipa`.`cod_equipa`;
