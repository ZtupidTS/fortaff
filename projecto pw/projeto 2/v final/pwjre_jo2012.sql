...
--
-- Host: localhost
-- Generation Time: Jan 29, 2012 at 07:14 PM
...
-- Procedures
--
CREATE DEFINER=`pwjre_root`@`localhost` PROCEDURE `wurfl_FallBackDevices`(current_fall_back VARCHAR(64))
BEGIN
WHILE current_fall_back != 'root' DO
	SELECT capabilities FROM wurflMerge WHERE deviceID = current_fall_back;
	SELECT fall_back FROM wurflMerge WHERE deviceID = current_fall_back INTO current_fall_back;
END WHILE;
END$$

CREATE DEFINER=`pwjre_root`@`localhost` PROCEDURE `wurfl_RIS`(IN ua VARCHAR(255), IN tolerance INT, IN matcher VARCHAR(64))
BEGIN
DECLARE curlen INT;
...
findua: WHILE ( curlen >= tolerance ) DO
	...
	SELECT idx.DeviceID INTO wurflid
		FROM wurflIndex idx INNER JOIN wurflMerge mrg ON idx.DeviceID = mrg.DeviceID
...
(1, 'Cerimónia', 'Abertura', '2012-07-27', '20:00:00', '02:30:00', 9.99, 1000, 500, 'V', 51.501904107618110, 0.131835937500000),
(2, 'Cerimónia', 'Encerramento', '2012-08-12', '20:00:00', '01:30:00', 9.99, 1000, 51, 'V', 51.501904107618110, 0.121835937500000),
(3, 'Cerimónia', 'Entrega de Medalhas 200M', '2012-08-02', '19:00:00', '01:00:00', 9.99, 1000, 150, 'V', 51.501904107618110, 0.111835937500000),
(7, 'Cerimónia', 'Fecho', '2012-08-18', '11:00:00', '02:00:00', 12.00, 20000, 0, 'V', 51.501904107618110, 0.101835937500000),
(8, 'Cerimónia', 'Bandeira', '2012-01-12', '12:00:00', '02:00:00', 0.00, 200, 0, 'V', 51.501904107618110, 0.108359375000000),
(9, 'Espetáculo', 'Palhaço', '2012-08-02', '14:00:00', '01:00:00', 0.00, 15000, 0, 'V', 51.501904107618110, 0.101835937500000),
(10, 'Espetáculo', 'Pombas', '2012-08-02', '15:00:00', '00:25:00', 0.00, 15000, 0, 'V', 51.501904107618110, 0.101835937500000);
...
INSERT INTO `opiniao` (`id_opiniao`, `tipo`, `email`, `conteudo`, `data`) VALUES
(1, 'CO', 'ricain59@gmail.com', 'dasdadadadadadadadad				', '2012-01-28'),
(2, 'CO', 'ricain59@gmail.com', 'dasdadadadadadadadad				', '2012-01-28'),
(3, 'CO', 'ricain59@gmail.com', '	dsdadadada					', '2012-01-28'),
(4, 'CO', 'ricain59@gmail.com', '	fdsfsfsfsfsdfs					', '2012-01-28'),
(5, 'SU', 'ricain59@gmail.com', 'A minha opinião é que o vosso site esta excelente.		', '2012-01-28');
...
(24, 12, 'Londres', '2012-08-05', '15:30:00', '02:00:00', 'Christian', 19.90, 55000, 1, 'V', 'M', 51.501904107618110, -0.158359375000000),
...
(5, 'ricardo', 'silva', 233999850, 'rua 24 de junho', 962411301, 'ricain59@gmail.com', 'jre1', 'jre1', '2012-01-29'),
(7, 'jorge', 'cunha', 111111111, 'braga', 969696969, 'jorgemcunham@msn.com', '1111', '1111', NULL);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `lugares_vazios_prova` AS select `prova`.`cod_prova` AS `cod_prova`,(`prova`.`lugares_total` - `prova`.`lugares_reservados`) AS `lugar_vazios` from `prova` WITH CASCADED CHECK OPTION;
