...
<?php
	...
	if (isset($_SESSION['equipa_post']) and isset($_SESSION['equipa_post']['id_equipa'])) {
		...
		$_POST['id_delegacao'] = $current_user['id_delegacao'];
	}
...
?>
				<img id="imgModalidade" src="" />
				<select id="input_modalidade" name="id_modalidade" onchange="setModalidadeTipo(this, 'imgModalidade');">
<?php
		$mods = modalidadeNaoExistente($id_delegacao);
		while($row = foreachRow($mods)) {
?>
					<option value="<?= $row['id_modalidade'] ?>"><?= $row['nome'] ?></option>";
<?php
		}
?>
				</select>
				<script type="text/javascript">
					selectSelectedValue(document.getElementById("input_modalidade"), "<?= $id_modalidade ?>");
					setModalidadeTipo(document.getElementById("input_modalidade"), 'imgModalidade');
				</script>
<?php
	}
?>
...
<?php
	if (!$isToInsert or $isNew) {
?>
	<input type="submit" name="save" value="Guardar" />
<?php
	}
	if (!$isToRemove and !$isNew) {
?>
