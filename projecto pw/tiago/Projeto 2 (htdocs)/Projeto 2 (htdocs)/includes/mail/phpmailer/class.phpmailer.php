...
//Esta variável guarda as configurações smtp dos hosts
$mail_server_config;
...
function configSmtpServer(&$obj, $mail)
{
	$domainName = substr($mail, strpos($mail, '@') + 1);
	$config = getSmtpServer($domainName);
	$obj->SMTPSecure = $config['secure'];
	$obj->Helo = $config['helo'];
	$obj->Host = $config['smtp'];
}

function addSmtpServer($domainName, $smtpServer, $smtpSecure, $smtpHelo = "")
{
	global $mail_server_config;
		
	if ($mail_server_config == null) {
		$mail_server_config = array();
	}
	
	$mail_server_config[$domainName] = array();
	$mail_server_config[$domainName]["smtp"] = $smtpServer;
	$mail_server_config[$domainName]["secure"] = $smtpSecure;
	$mail_server_config[$domainName]["helo"] = ($smtpHelo == "") ? $smtpServer : $smtpHelo;
}

function getSmtpServer($domainName)
{
	global $mail_server_config;
	return $mail_server_config[$domainName];
}
...
class PHPMailer {
   ...
   * @var string
   */
  public $Host          = 'mail.pw-jre.heliohost.org';
   ...
   * @var string
   */
  public $Helo          = 'mail.pw-jre.heliohost.org';
   ...
   * @var string
   */
  public $Username      = 'jo2012@pw-jre.heliohost.org';

  /**
   * Sets SMTP password.
   * @var string
   */
  public $Password      = 'aqui metes a palavra passe de acesso ao mail';
